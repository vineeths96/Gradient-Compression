...
class AverageMeter:
    ...
    def reduce(self):
        ...
        total_count = torch.tensor(
            self._counter, dtype=torch.int32, device=self._device
        )
        count_reduce_op = torch.distributed.all_reduce(
            tensor=total_count, async_op=True
        )
        count_reduce_op.wait()

        for key in self._average:
            tensor = torch.tensor(
                self._average[key], dtype=torch.float16, device=self._device
            )
